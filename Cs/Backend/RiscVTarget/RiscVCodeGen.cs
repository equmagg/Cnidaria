        ...
        private sealed class Generator
        {
            ...
            private GenTreeMethod SelectEntryMethod()
            {
                ...
                foreach (var method in _program.Methods)
                {
                    ...
                    if (StringComparer.Ordinal.Equals(runtimeMethod.Name, "<Main>$") &&
                        runtimeMethod.IsStatic &&
                        (runtimeMethod.ParameterTypes.Length == 0 ||
            ...
            public void EmitLoadImmediate(RVRegister destination, long value)
            {
                ...
                if (value >= int.MinValue && value <= int.MaxValue)
                {
                    ...
                    if (lo != 0)
                        Emit(RVInstruction.I(_machineTarget.Is64Bit ? RVInstrKind.Addiw : RVInstrKind.Addi, destination, destination, lo));
                    return;
                }
            ...
            private void EmitTypeInitializationThunk(TypeInitializationThunkDraft thunk, string? waitHelperLabel)
            {
                ...
                bool hasZacas = _machineTarget.HasZacas;
                bool hasZalrsc = _machineTarget.HasZalrsc;
                bool hasAtomics = hasZacas || hasZalrsc;
                ...
                if (hasAtomics)
                {
                    ...
                    if (hasZacas)
                    {
                        EmitLoadImmediate(RVRegister.X31, 1);
                        EmitLoadImmediate(RVRegister.X29, 2);
                        Emit(RVInstruction.Amo(
                            RVInstrKind.AmocasW,
                            RVRegister.X31,
                            RVRegister.X28,
                            RVRegister.X29,
                ...
                if (hasAtomics)
                {
                    if (_machineTarget.HasZaamo)
                    {
                        Emit(RVInstruction.Amo(RVInstrKind.AmoSwapW, RVRegister.X0, RVRegister.X28, RVRegister.X0, release: true));
                    }
                    else
                    {
                        string publishRetryLabel = CreateLocalLabel(thunk.Label + "_publish_retry");
                        DefineLabel(publishRetryLabel);
                        Emit(RVInstruction.Amo(RVInstrKind.LrW, RVRegister.X31, RVRegister.X28, RVRegister.X0));
                        Emit(RVInstruction.Amo(RVInstrKind.ScW, RVRegister.X30, RVRegister.X28, RVRegister.X0, release: true));
                        Emit(RVInstruction.B(RVInstrKind.Bne, RVRegister.X30, RVRegister.X0, publishRetryLabel));
                    }
                    EmitMaterializeAddress(thunk.StateLabel, RVRegister.X28);
            ...
            {
                if (_target.OperatingSystem == OperatingSystemKind.Linux &&
                    !_machineTarget.HasZacas &&
                    !_machineTarget.HasZalrsc)
                {
                    throw new NotSupportedException("Thread-safe static initialization on Linux RISC-V requires Zacas or Zalrsc.");
                }
            ...
            private sealed class MethodEmitter
            {
                ...
                private bool MethodHasGcSafePoint()
                {
                    for (int i = 0; i < _method.LinearNodes.Length; i++)
                    {
                        ...
                        if (node.TreeKind == GenTreeKind.Call)
                        {
                            ...
                            if (method is null)
                                return true;
                            if (method.HasInternalCall != true || RiscVRuntime.IsGcSafePointInternalCall(method))
                                return true;
                        }
                    }
                    return false;
                ...
                private void EmitMove(GenTree node)
                {
                    ...
                    NormalizeScalarizedAggregateMove(destination, source, ref type, ref kind);
                    ...
                    bool single)
                {
                    ...
                    {
                        throw Unsupported(node, "floating remainder requires floating-point registers");
                    }
                    if (!single && !MachineTarget.Is64Bit)
                        throw Unsupported(node, "double floating remainder on RV32 is not implemented");

                    GenStackKind kind = single ? GenStackKind.R4 : GenStackKind.R8;
                    const MachineRegister argument0 = MachineRegister.F10;
                    const MachineRegister argument1 = MachineRegister.F11;
                    const MachineRegister scratch = MachineRegister.F29;

                    if (left == argument1 && right == argument0)
                    {
                        EmitRegisterMove(scratch, left, null, kind);
                        EmitRegisterMove(argument1, right, null, kind);
                        EmitRegisterMove(argument0, scratch, null, kind);
                    }
                    else if (right == argument0 && left != argument0)
                    {
                        EmitRegisterMove(argument1, right, null, kind);
                        EmitRegisterMove(argument0, left, null, kind);
                    }
                    else
                    {
                        if (left != argument0)
                    ...
                    string symbol = single
                        ? RiscVRuntime.FloatingRemainderSingleSymbol
                        : RiscVRuntime.FloatingRemainderDoubleSymbol;
                    _owner.EmitPcrelTransfer(_owner.ResolveExternalSymbol(symbol), link: true);

                    if (destination != argument0)
                        EmitRegisterMove(destination, argument0, null, kind);
                }
                ...
                private void EmitHiddenReturnBufferCopy(GenTree node)
                {
                    ...
                    RegisterOperand hiddenHome = RegisterOperand.ForFrameSlot(
                       ...
                       hiddenSlot.Offset,
                        hiddenSlot.Size);
                    MachineRegister returnRegister = RegisterInfo.GetIntegerReturnRegister(Target, 0);
                    RVRegister returnBuffer = ToIntegerRegister(returnRegister);
                    EmitMemoryLoad(
                        returnRegister,
                       FrameBase(hiddenHome),
                        EffectiveFrameOffset(hiddenHome),
                        Target.PointerSize,
                        signed: false);
                ...
                private void EmitCall(GenTree node)
                {
                    ...
                    if (method.HasInternalCall)
                    {
                        ...
                        if (RiscVRuntime.IsAllocateNewArrayInternalCall(method))
                        {
                ...
                private void EmitInterlockedExchangeAdd(GenTree node, InterlockedExchangeAddIntrinsic intrinsic)
                {
                    if (!MachineTarget.HasZaamo && !MachineTarget.HasZalrsc)
                        throw Unsupported(node, "Interlocked.ExchangeAdd requires the RISC-V Zaamo or Zalrsc extension");
                    if (intrinsic.Size == 8 && !MachineTarget.Is64Bit)
                        throw Unsupported(node, "64-bit Interlocked.ExchangeAdd is not supported on RV32");
                    ...
                    if (MachineTarget.HasZaamo)
                    {
                        _owner.Emit(RVInstruction.Amo(
                            intrinsic.Size == 8 ? RVInstrKind.AmoAddD : RVInstrKind.AmoAddW,
                            RVRegister.X10,
                            RVRegister.X28,
                            value,
                            acquire: true,
                    ...
                    {
                        string retry = _owner.CreateLocalLabel($"{_methodLabel}_exchange_add_retry_{node.LinearId}");
                        RVRegister addend = value;
                        if (addend == RVRegister.X10)
                        {
                            _owner.EmitMove(RVRegister.X30, addend);
                            addend = RVRegister.X30;
                        }
                        ...
                        _owner.Emit(RVInstruction.Amo(
                            ...
                            acquire: true));
                        _owner.Emit(RVInstruction.R(
                            intrinsic.Size == 4 && MachineTarget.Is64Bit ? RVInstrKind.Addw : RVInstrKind.Add,
                            RVRegister.X29,
                            RVRegister.X10,
                            addend));
                        _owner.Emit(RVInstruction.Amo(
                ...
                {
                    if (!MachineTarget.HasZacas && !MachineTarget.HasZalrsc)
                        throw Unsupported(node, "Interlocked.CompareExchange requires the RISC-V Zacas or Zalrsc extension");

                    if (intrinsic.Size == 8 && MachineTarget.Is32Bit)
                    {
                        EmitInterlockedCompareExchangeRv32D(node);
                        return;
                    }

                    if (node.Uses.Length != 3 || !node.Uses[0].IsRegister || !node.Uses[1].IsRegister || !node.Uses[2].IsRegister)
                        throw Unsupported(node, "Interlocked.CompareExchange requires three scalar ABI register operands");

                    RVRegister location = ToIntegerRegister(node.Uses[0].Register);
                    RVRegister value = ToIntegerRegister(node.Uses[1].Register);
                    RVRegister comparand = ToIntegerRegister(node.Uses[2].Register);
                    if ((node.Flags & GenTreeFlags.NullCheckEliminated) == 0)
                    ...
                    if (MachineTarget.HasZacas)
                        EmitInterlockedCompareExchangeZacas(node, intrinsic, value, comparand);
                    else
                        EmitInterlockedCompareExchangeLrSc(node, intrinsic, value, comparand);

                    _owner.Emit(new RVInstruction(RVInstrKind.Fence, immediate: 0x33));
                }

                private void EmitInterlockedCompareExchangeRv32D(GenTree node)
                {
                    if (!MachineTarget.HasZacas)
                        throw Unsupported(node, "64-bit Interlocked.CompareExchange on RV32 requires Zacas");
                    if (node.Uses.Length != 5)
                        throw Unsupported(node, "64-bit Interlocked.CompareExchange on RV32 requires five ABI register operands");
                    for (int i = 0; i < node.Uses.Length; i++)
                    {
                        if (!node.Uses[i].IsRegister)
                            throw Unsupported(node, "64-bit Interlocked.CompareExchange on RV32 requires register ABI operands");
                    }

                    RVRegister location = ToIntegerRegister(node.Uses[0].Register);
                    RVRegister valueLo = ToIntegerRegister(node.Uses[1].Register);
                    RVRegister valueHi = ToIntegerRegister(node.Uses[2].Register);
                    RVRegister comparandLo = ToIntegerRegister(node.Uses[3].Register);
                    RVRegister comparandHi = ToIntegerRegister(node.Uses[4].Register);

                    if ((node.Flags & GenTreeFlags.NullCheckEliminated) == 0)
                        EmitNullCheck(node, location, "interlocked_compare_exchange");

                    _owner.EmitMove(RVRegister.X28, location);
                    _owner.EmitMove(RVRegister.X30, valueLo);
                    _owner.EmitMove(RVRegister.X31, valueHi);
                    _owner.EmitMove(RVRegister.X10, comparandLo);
                    _owner.EmitMove(RVRegister.X11, comparandHi);
                    _owner.Emit(new RVInstruction(RVInstrKind.Fence, immediate: 0x33));
                    _owner.Emit(RVInstruction.Amo(
                        RVInstrKind.AmocasD,
                        RVRegister.X10,
                ...
                private void EmitInterlockedCompareExchangeZacas(
                    GenTree node,
                    InterlockedCompareExchangeIntrinsic intrinsic,
                    RVRegister value,
                    RVRegister comparand)
                {
                    if (intrinsic.Size == 8)
                    {
                        RVRegister casValue = value;
                        if (casValue == RVRegister.X10)
                        {
                            _owner.EmitMove(RVRegister.X30, casValue);
                            casValue = RVRegister.X30;
                        }
                        _owner.EmitMove(RVRegister.X10, comparand);
                        _owner.Emit(RVInstruction.Amo(
                            RVInstrKind.AmocasD,
                            RVRegister.X10,
                            RVRegister.X28,
                    ...
                    if (intrinsic.Size == 4)
                    {
                        ...
                        if (MachineTarget.Is64Bit)
                            _owner.Emit(RVInstruction.I(RVInstrKind.Addiw, RVRegister.X10, comparand, 0));
                        else
                            _owner.EmitMove(RVRegister.X10, comparand);
                        ...
                        if (MachineTarget.Is64Bit && !intrinsic.IsSigned && !intrinsic.IsReference)
                            EmitZeroExtend32(RVRegister.X10, RVRegister.X10);
                        return;
                    }

                    if (intrinsic.Size is not (1 or 2))
                        throw Unsupported(node, $"Unsupported Interlocked.CompareExchange size {intrinsic.Size}");

                    EmitInterlockedCompareExchangeSubwordZacas(node, intrinsic, value, comparand);
                }

                private void EmitInterlockedCompareExchangeSubwordZacas(
                    GenTree node,
                    InterlockedCompareExchangeIntrinsic intrinsic,
                    RVRegister value,
                    RVRegister comparand)
                {
                    ...
                    string retry = _owner.CreateLocalLabel($"{_methodLabel}_cmpxchg_zacas_retry_{node.LinearId}");
                    string done = _owner.CreateLocalLabel($"{_methodLabel}_cmpxchg_zacas_done_{node.LinearId}");
                    ...
                    _owner.Emit(RVInstruction.R(RVInstrKind.And, RVRegister.X14, comparand, RVRegister.X30));
                    _owner.Emit(RVInstruction.R(RVInstrKind.Sll, RVRegister.X14, RVRegister.X14, RVRegister.X29));
                    ...
                    _owner.Emit(RVInstruction.I(RVInstrKind.Lw, RVRegister.X16, RVRegister.X28, 0));

                    _owner.DefineLabel(retry);
                    _owner.Emit(RVInstruction.R(RVInstrKind.And, RVRegister.X10, RVRegister.X16, RVRegister.X30));
                    _owner.Emit(RVInstruction.B(RVInstrKind.Bne, RVRegister.X10, RVRegister.X14, done));
                    ...
                    _owner.EmitMove(RVRegister.X31, RVRegister.X16);
                    _owner.Emit(RVInstruction.Amo(
                        RVInstrKind.AmocasW,
                        RVRegister.X16,
                        RVRegister.X28,
                        ...
                        acquire: true,
                        release: true));
                    _owner.Emit(RVInstruction.B(RVInstrKind.Bne, RVRegister.X16, RVRegister.X31, retry));
                    _owner.DefineLabel(done);
                ...
                private void EmitInterlockedCompareExchangeLrSc(
                    GenTree node,
                    InterlockedCompareExchangeIntrinsic intrinsic,
                    RVRegister value,
                    RVRegister comparand)
                {
                    string retry = _owner.CreateLocalLabel($"{_methodLabel}_cmpxchg_retry_{node.LinearId}");
                    string done = _owner.CreateLocalLabel($"{_methodLabel}_cmpxchg_done_{node.LinearId}");

                    if (intrinsic.Size == 8)
                    {
                        if (!MachineTarget.Is64Bit)
                            throw Unsupported(node, "64-bit Interlocked.CompareExchange is not supported on RV32");

                        _owner.DefineLabel(retry);
                        _owner.Emit(RVInstruction.Amo(RVInstrKind.LrD, RVRegister.X10, RVRegister.X28, RVRegister.X0, acquire: true));
                        _owner.Emit(RVInstruction.B(RVInstrKind.Bne, RVRegister.X10, comparand, done));
                        _owner.Emit(RVInstruction.Amo(RVInstrKind.ScD, RVRegister.X31, RVRegister.X28, value, release: true));
                        _owner.Emit(RVInstruction.B(RVInstrKind.Bne, RVRegister.X31, RVRegister.X0, retry));
                        _owner.DefineLabel(done);
                        return;
                    }
                    ...
                    if (intrinsic.Size == 4)
                    {
                        RVRegister normalizedComparand = comparand;
                        if (MachineTarget.Is64Bit)
                        {
                            _owner.Emit(RVInstruction.I(RVInstrKind.Addiw, RVRegister.X29, comparand, 0));
                            normalizedComparand = RVRegister.X29;
                        }

                        _owner.DefineLabel(retry);
                        _owner.Emit(RVInstruction.Amo(RVInstrKind.LrW, RVRegister.X10, RVRegister.X28, RVRegister.X0, acquire: true));
                        _owner.Emit(RVInstruction.B(RVInstrKind.Bne, RVRegister.X10, normalizedComparand, done));
                        _owner.Emit(RVInstruction.Amo(RVInstrKind.ScW, RVRegister.X31, RVRegister.X28, value, release: true));
                        _owner.Emit(RVInstruction.B(RVInstrKind.Bne, RVRegister.X31, RVRegister.X0, retry));
                        _owner.DefineLabel(done);
                        if (MachineTarget.Is64Bit && !intrinsic.IsSigned && !intrinsic.IsReference)
                            EmitZeroExtend32(RVRegister.X10, RVRegister.X10);
                ...
                private void EmitAllocateNewArray(GenTree node, RuntimeMethod method)
                {
                    ...
                    _owner.EmitMaterializeAddress(_owner.GetTypeDescriptorLabel(arrayType), RVRegister.X10);
                    MarkEhCallSite(node, "allocate_uninitialized_array");
                    _owner.EmitPcrelTransfer(_owner.ResolveExternalSymbol(RiscVRuntime.NewUninitializedArraySymbol), link: true);
                    _owner.DefineLabel(safePoint.ReturnLabel);
                }
                ...
                private void EmitExplicitNullCheck(GenTree node)
                {
                    ...
                    EmitNullCheck(node, ToIntegerRegister(node.Uses[0].Register), "explicit_nullcheck");
                }
                ...
                private void EmitArrayLength(GenTree node)
                {
                    ...
                    RVRegister array = ToIntegerRegister(RequireUseRegisterForOperand(node, 0, "array length"));
                    if ((node.Flags & GenTreeFlags.NullCheckEliminated) == 0)
                        EmitArrayNullCheck(node, array);

                    EmitMemoryLoad(node.Results[0].Register, array, Target.ArrayLengthOffset, 4, signed: true);
                }
                ...
                private void PreserveArrayElementOperands(
                    GenTree node,
                    out RVRegister array,
                    out RVRegister index)
                {
                    int arrayUseIndex = RequireCodegenUseIndexForOperand(node, 0, "array operand");
                    ...
                    array = ToIntegerRegister(RequireInternalGeneralRegister(node, 0));
                    index = ToIntegerRegister(RequireInternalGeneralRegister(node, 1));
                    _owner.EmitMove(array, ToIntegerRegister(node.Uses[arrayUseIndex].Register));
                    _owner.EmitMove(index, ToIntegerRegister(node.Uses[indexUseIndex].Register));
                }
                ...
                private void EmitArrayElementStore(GenTree node, RuntimeType elementType)
                {
                    ...
                    AbiValueInfo abi = MachineAbi.ClassifyStorageValue(elementType, kind, Target);
                    RegisterOperand value = node.Uses[valueUseIndex];

                    MachineRegister preservedArrayRegister = RequireInternalGeneralRegister(node, 0);
                    MachineRegister preservedIndexRegister = RequireInternalGeneralRegister(node, 1);
                    RVRegister array = ToIntegerRegister(preservedArrayRegister);
                    RVRegister index = ToIntegerRegister(preservedIndexRegister);
                    _owner.EmitMove(array, ToIntegerRegister(node.Uses[arrayUseIndex].Register));
                    _owner.EmitMove(index, ToIntegerRegister(node.Uses[indexUseIndex].Register));

                    MachineRegister preservedValueRegister = MachineRegister.Invalid;
                    if (abi.PassingKind == AbiValuePassingKind.ScalarRegister &&
                         abi.RegisterClass == RegisterClass.General &&
                         value.IsRegister &&
                        value.RegisterClass == RegisterClass.General)
                    {
                        preservedValueRegister = RequireInternalGeneralRegister(node, 2);
                        _owner.EmitMove(
                             ToIntegerRegister(preservedValueRegister),
                    ...
                    if (elementType.IsReferenceType)
                    {
                        ...
                        if (preservedValueRegister == MachineRegister.Invalid)
                            throw Unsupported(node, "Reference array store value was not preserved");
                        EmitArrayReferenceStoreCheck(node, array, ToIntegerRegister(preservedValueRegister));
                    }

                    if (preservedValueRegister != MachineRegister.Invalid)
                    {
                        EmitMemoryStore(
                            preservedValueRegister,
                            RVRegister.X28,
                            0,
                            StorageSize(elementType, kind, value));
                        return;
                    }
                ...
                private void EmitArrayElementAddress(
                    ...
                    RVRegister arrayOverride = RVRegister.Invalid,
                    RVRegister indexOverride = RVRegister.Invalid)
                {
                    RVRegister array;
                    if (arrayOverride != RVRegister.Invalid)
                    {
                        array = arrayOverride;
                    }
                    else
                    {
                        ...
                        if (!node.Uses[arrayUseIndex].IsRegister)
                            throw Unsupported(node, "Array element address requires a register array operand");
                        array = ToIntegerRegister(node.Uses[arrayUseIndex].Register);
                    }

                    RVRegister index;
                    if (indexOverride != RVRegister.Invalid)
                    {
                        index = indexOverride;
                    }
                    else
                    {
                        ...
                        if (!node.Uses[indexUseIndex].IsRegister)
                            throw Unsupported(node, "Array element address requires a register index operand");
                        index = ToIntegerRegister(node.Uses[indexUseIndex].Register);
                    }
                ...
                private void EmitValueToAddress(GenTree node, int firstUseIndex, RuntimeType type, GenStackKind kind, RVRegister address)
                {
                ...
                private void EmitNewStringObject(GenTree node, RuntimeMethod constructor, RegisterOperand objectHome)
                {
                    ...
                    {
                        runtimeSymbol = RiscVRuntime.NewStringFromReadOnlySpanSymbol;
                    }
                    else
                    {
                ...
                private void EmitField(GenTree node)
                {
                    ...
                    if (node.TreeKind == GenTreeKind.StoreField && TryEmitSsaPromotedFieldDefinition(node, field))
                    {
                        return;
                    }
                ...
                private bool TryEmitSsaPromotedFieldDefinition(GenTree node, RuntimeField field)
                {
                    if (!node.SsaStoreTargetName.HasValue)
                        return false;

                    if (node.LocalDescriptor is not { IsStructField: true })
                        return false;
                    ...
                    RuntimeType fieldType = field.FieldType;
                    GenStackKind fieldKind = StackKindForType(fieldType);
                        ...
                        node.Operands[1].Kind == GenTreeKind.DefaultValue)
                    {
                        if (node.Results.Length > 1)
                        {
                            var abi = MachineAbi.ClassifyStorageValue(fieldType, fieldKind, Target);
                            var segments = MachineAbi.GetRegisterSegments(abi, Target);
                            if (segments.Length != node.Results.Length)
                            {
                                throw Unsupported(
                                    node,
                                    "SSA promoted field default definition fragment count does not match storage ABI");
                            }
                            ...
                            for (int i = 0; i < segments.Length; i++)
                            {
                                EmitZeroOperand(
                                    ...
                                    node.Results[i],
                                    null,
                                    StackKindForSegment(segments[i]),
                                    segments[i].Size);
                            }
                        }
                        else if (node.Results.Length == 1)
                        {
                            EmitZeroOperand(
                                ...
                                fieldType,
                                fieldKind,
                                StorageSize(fieldType, fieldKind, node.Results[0]));
                        }
                    ...
                    if (node.Results.Length != node.Uses.Length)
                    {
                        throw Unsupported(
                            node,
                            "SSA promoted field definition has mismatched source and destination fragment counts");
                    }
                    ...
                    if (node.Results.Length > 1)
                    {
                        ...
                        if (segments.Length != node.Results.Length)
                        {
                            throw Unsupported(
                                node,
                                "SSA promoted field definition fragment count does not match storage ABI");
                        }
                    ...
                    {
                        EmitMoveBetween(node, node.Results[0], node.Uses[0], fieldType, fieldKind);
                    }
                ...
                private void EmitStaticField(GenTree node)
                {
                    ...
                    RuntimeType fieldType = field.FieldType;
                    GenStackKind kind = StackKindForType(fieldType);
                    _owner.EmitMaterializeAddress(_owner.GetOrCreateStaticStorage(field.DeclaringType).StorageLabel, RVRegister.X28);
                    if (field.Offset != 0)
