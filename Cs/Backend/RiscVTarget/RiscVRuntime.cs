...
namespace Cnidaria.Cs
{
    internal static class RiscVRuntime
    {
        ...
        private static RiscVProgram Compile(TargetInfo target)
        {
            ...
            string source = BuildSource(target.PointerSize);
        ...
        private static string BuildSource(int pointerSize)
        {
            int stringLengthOffset = pointerSize;
            int stringCharsOffset = checked(stringLengthOffset + 4);
            int arrayLengthOffset = pointerSize;
            int arrayDataOffset = checked(arrayLengthOffset + (pointerSize == 8 ? 8 : 4));
            return $"#define POINTER_SIZE {pointerSize}\n" +
                    $"#define SYNC_BLOCK_SIZE {pointerSize}\n" +
                    $"#define STRING_LENGTH_OFFSET {stringLengthOffset}\n" +
                    $"#define STRING_CHARS_OFFSET {stringCharsOffset}\n" +
                    $"#define ARRAY_LENGTH_OFFSET {arrayLengthOffset}\n" +
                    $"#define ARRAY_DATA_OFFSET {arrayDataOffset}\n" +
                    ReadRuntimeSource("CLRSource.c");
        }
    }
}
