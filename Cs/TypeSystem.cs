...
namespace Cnidaria.Cs
{
    sealed class RuntimeModule
    {
        ...
        public Dictionary<int, BytecodeFunction> MethodsByDefToken { get; }
        ...
        public RuntimeModule(string name, IMetadataView md, Dictionary<int, BytecodeFunction> methodsByDefToken)
        {
            Name = name;
        ...
        private (string ns, string name) GetTypeDefFullNameByRid(int rid)
        {
    ...
    sealed class Domain
    {
        ...
        public (RuntimeModule? module, BytecodeFunction fn) ResolveCall(RuntimeModule caller, int methodToken)
        {
    ...
    internal sealed class RuntimeTypeSystem
    {
        ...
        public RuntimeMethod ResolveMethod(RuntimeModule module, int methodToken)
        {
            ...
            if ((cc & 0x10) != 0)
                throw new NotSupportedException("Generic method MemberRef signatures are not supported.");
            ...
            for (int i = 0; i < owner.Methods.Length; i++)
            {
                ...
                if (m.HasThis != hasThis)
                    continue;
                if (!ReferenceEquals(m.ReturnType, ret))
                    continue;
        ...
        public bool TryResolveMethod(RuntimeModule module, int methodToken, out RuntimeMethod? m)
        {
            if (_methodCache.TryGetValue((module.Name, methodToken), out m))
                return true;

            try
            {
                m = ResolveMethod(module, methodToken);
                return true;
            }
            catch
            {
        ...
        private RuntimeMethod GetOrCreateConstructedMethod(RuntimeMethod genericMethod, RuntimeType[] methodArgs)
        {
            ...
            var m = new RuntimeMethod(
                ...
                declType: genericMethod.DeclaringType,
        ...
        private void EnsureLayout(RuntimeType? t)
        {
            ...
            if (TryGetPrimitiveLayout(t, out int primSize, out int primAlign))
            {
                t.SizeOf = primSize;
                t.AlignOf = primAlign;
                t.InstanceSize = t.IsReferenceType ? PointerSize : primSize;
        ...
        {
            if (((TypeAttributes)td.Flags & TypeAttributes.Interface) != 0)
