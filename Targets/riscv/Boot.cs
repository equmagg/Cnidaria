...
public sealed class RiscVDeviceTreeLayout
{
    ...
    public ulong HostBridgeSize { get; set; } = RVMmioHostBridge.RegisterWindowSize;
    public bool FramebufferEnabled { get; set; } = true;
    public ulong FramebufferBase { get; set; } = 0x50000000UL;
    public ulong FramebufferSize { get; set; } = RVFramebuffer.RegisterWindowSize + 640UL * 480UL * 4UL;
    public uint FramebufferWidth { get; set; } = 640;
    public uint FramebufferHeight { get; set; } = 480;
...
public static class RiscVZBoot
{
        ...
        byte[]? hostLibrary = null)
    {
        ...
        byte[] dtb = BuildDefaultDeviceTree(new RiscVDeviceTreeLayout
        {
            ...
            HostBridgeEnabled = machine.HostBridge != null,
            HostBridgeBase = machine.HostBridge?.BaseAddress ?? 0x1000A000UL,
            FramebufferEnabled = machine.Framebuffer != null,
            FramebufferBase = machine.Framebuffer?.BaseAddress ?? 0x50000000UL,
            FramebufferSize = machine.Framebuffer?.WindowSize ?? 0UL,
            FramebufferWidth = (uint)(machine.Framebuffer?.Width ?? 0),
            FramebufferHeight = (uint)(machine.Framebuffer?.Height ?? 0),
            BootHartId = checked((uint)machine.HartId),
        });
        var assemblySettings = CreateAssemblySettings(layout);

        var bootFiles = MergeDefaultUserlandFiles(additionalBootFiles, autorunSource, hostLibrary);
    ...
    private static IReadOnlyList<RiscVBootFile> MergeDefaultUserlandFiles(
        IEnumerable<RiscVBootFile>? additionalBootFiles, byte[]? autorunSource, byte[]? hostLibrary)
    {
        var files = RiscVUserland.BuildDefaultBootFiles(autorunSource, hostLibrary).ToList();
    ...
    public static byte[] BuildDefaultDeviceTree(RiscVDeviceTreeLayout? layout = null)
    {
        ...
        if (layout.HostBridgeEnabled)
        {
            fdt.BeginNode("hostbridge@" + layout.HostBridgeBase.ToString("x", CultureInfo.InvariantCulture));
            fdt.PropString("compatible", "cnidaria,host-bridge");
            fdt.PropReg(layout.HostBridgeBase, layout.HostBridgeSize);
            fdt.EndNode();
        }

        if (layout.FramebufferEnabled)
        {
            fdt.BeginNode("framebuffer@" + layout.FramebufferBase.ToString("x", CultureInfo.InvariantCulture));
            fdt.PropString("compatible", "cnidaria,framebuffer");
            fdt.PropReg(layout.FramebufferBase, layout.FramebufferSize);
            fdt.PropU32("width", layout.FramebufferWidth);
            fdt.PropU32("height", layout.FramebufferHeight);
            fdt.PropString("format", "a8r8g8b8");
            fdt.EndNode();
        }
...
public struct RiscVUserlandLayout
{
    ...
    public ulong InterpreterBase { get; set; } = 0x2000000000UL;
    public RiscVUserlandLayout()
    {
    }
    public RiscVUserlandLayout(ulong imageBase)
    {
        ImageBase = imageBase;
        InterpreterBase = 0x2000000000UL;
    }
    public static bool operator ==(RiscVUserlandLayout left, RiscVUserlandLayout right)
        => left.ImageBase == right.ImageBase && left.InterpreterBase == right.InterpreterBase;
    public static bool operator !=(RiscVUserlandLayout left, RiscVUserlandLayout right)
        => !(left == right);
    public override bool Equals(object? obj)
        => obj is RiscVUserlandLayout l && this == l;
    public override int GetHashCode() => ImageBase.GetHashCode() ^ InterpreterBase.GetHashCode();
}
...
public static class RiscVUserland
{
    ...
    public static string FramebufferLibrarySource => ReadUserSource("fblib.c");
    ...
    public const string InterpreterFileName = "LD.SO";
    public const string StandardLibrarySoName = "libc.so";
    public const string StandardLibraryFileName = "LIBC.SO";
    public const string HostLibrarySoName = "libhost.so";
    public const string HostLibraryFileName = "LIBHOST.SO";
    public const string FramebufferLibrarySoName = "libfb.so";
    public const string FramebufferLibraryFileName = "LIBFB.SO";

    private static readonly Lazy<byte[]> LazyDynamicLoader = new(() => BuildDynamicLoader());
    private static readonly Lazy<SharedObjectArtifact> LazyStandardLibrary = new(BuildSharedStandardLibrary);
    private static readonly Lazy<SharedObjectArtifact> LazyHostLibrary = new(BuildHostLibrary);
    private static readonly Lazy<SharedObjectArtifact> LazyFramebufferLibrary = new(BuildFramebufferLibrary);

    public static byte[] DefaultDynamicLoader => LazyDynamicLoader.Value;
    public static byte[] DefaultSharedStandardLibrary => LazyStandardLibrary.Value.Image;
    public static ImmutableArray<string> SharedStandardLibraryExports => LazyStandardLibrary.Value.Exports;
    public static byte[] DefaultHostLibrary => LazyHostLibrary.Value.Image;
    public static ImmutableArray<string> HostLibraryExports => LazyHostLibrary.Value.Exports;
    public static byte[] DefaultFramebufferLibrary => LazyFramebufferLibrary.Value.Image;
    public static ImmutableArray<string> FramebufferLibraryExports => LazyFramebufferLibrary.Value.Exports;
    ...
    public static Cnidaria.C.StaticLinkResult<RiscVProgram> LinkDynamicObject(
        IEnumerable<Cnidaria.C.SourceFile> sources,
        IEnumerable<string>? needed = null,
        IEnumerable<string>? additionalImports = null,
        Cnidaria.C.TargetInfo? target = null)
    {
        if (sources is null)
            throw new ArgumentNullException(nameof(sources));
        var options = new Cnidaria.C.StaticLinkerOptions(
            target ?? Cnidaria.C.TargetInfo.RV64GLinux,
            includeFiles: Cnidaria.C.StandardHeaders.CreateFiles(),
            allowedUndefinedSymbols: ResolveImports(needed, additionalImports),
            linkStandardLibrary: false);
        return Cnidaria.C.StaticLinker.LinkRiscV(sources, options);
    }
        ...
        RiscVUserlandLayout? layout = null)
    {
        ...
        return RiscVElfWriter.WriteDynamicExecutable(
            program,
            layout.Value.ImageBase,
            InterpreterPath,
            ResolveDependencies(needed));
    }

    public static byte[] LinkDynamicProgram(
        IEnumerable<Cnidaria.C.SourceFile> sources,
        IEnumerable<string>? needed = null,
        RiscVUserlandLayout? layout = null,
        IEnumerable<string>? additionalImports = null,
        Cnidaria.C.TargetInfo? target = null)
    {
        var linked = LinkDynamicObject(sources, needed, additionalImports, target);
        return WriteDynamicExecutable(RequireProgram(linked, "dynamic program"), needed, layout);
    }

    public static byte[] LinkSharedLibrary(
        IEnumerable<Cnidaria.C.SourceFile> sources,
        string soName,
        IEnumerable<string>? needed = null,
        ...
        Cnidaria.C.TargetInfo? target = null)
    {
        ...
        if (string.IsNullOrWhiteSpace(soName))
            throw new ArgumentException("A shared library needs a name.", nameof(soName));
        var program = LinkLibraryObject(sources, ResolveImports(needed, null), soName, target);
        return RiscVElfWriter.WriteSharedObject(program, soName, ResolveDependencies(needed, false), initSymbol);
    }

    private static ImmutableArray<string> ResolveDependencies(IEnumerable<string>? needed, bool standardLibraryByDefault = true)
    {
        if (needed is not null)
            return needed.ToImmutableArray();
        return standardLibraryByDefault
            ? ImmutableArray.Create(StandardLibrarySoName)
            : ImmutableArray<string>.Empty;
    }

    private static ImmutableArray<string> ResolveImports(IEnumerable<string>? needed, IEnumerable<string>? additionalImports)
    {
        var dependencies = ResolveDependencies(needed);
        var imports = ImmutableArray<string>.Empty;
        if (dependencies.Contains(StandardLibrarySoName, StringComparer.Ordinal))
            imports = imports.AddRange(SharedStandardLibraryExports);
        if (dependencies.Contains(HostLibrarySoName, StringComparer.Ordinal))
            imports = imports.AddRange(HostLibraryExports);
        if (dependencies.Contains(FramebufferLibrarySoName, StringComparer.Ordinal))
            imports = imports.AddRange(FramebufferLibraryExports);
        return additionalImports is null ? imports : imports.AddRange(additionalImports);
    }

    private static RiscVProgram LinkLibraryObject(
        IEnumerable<Cnidaria.C.SourceFile> sources,
        ImmutableArray<string> allowedUndefinedSymbols,
        string what,
        Cnidaria.C.TargetInfo? target = null)
    {
        var options = new Cnidaria.C.StaticLinkerOptions(
            target ?? Cnidaria.C.TargetInfo.RV64GLinux,
            includeFiles: Cnidaria.C.StandardHeaders.CreateFiles(),
            allowedUndefinedSymbols: allowedUndefinedSymbols,
            linkStandardLibrary: false,
            emitStartup: false);
        return RequireProgram(Cnidaria.C.StaticLinker.LinkRiscV(sources, options), what);
    }

    public static byte[] BuildDynamicLoader(RiscVUserlandLayout? layout = null)
    {
        layout ??= new RiscVUserlandLayout();
        var program = CompileProgram(DynamicLoaderSource, "riscv/os/ldso.c");
        return program.ToLinuxExecutableBytes(layout.Value.InterpreterBase);
    }

    private static SharedObjectArtifact BuildFramebufferLibrary()
    {
        var program = LinkLibraryObject(
            new[] { new Cnidaria.C.SourceFile("riscv/os/fblib.c", FramebufferLibrarySource) },
            ImmutableArray<string>.Empty,
            FramebufferLibrarySoName,
            Cnidaria.C.TargetInfo.RVA32Linux);
        return new SharedObjectArtifact(
            RiscVElfWriter.WriteSharedObject(program, FramebufferLibrarySoName, initSymbol: "__fb_init"),
            ExportedNames(program));
    }

    private static SharedObjectArtifact BuildHostLibrary()
    {
        var program = LinkLibraryObject(
            new[] { new Cnidaria.C.SourceFile("riscv/os/hostlib.c", HostLibrarySource) },
            ImmutableArray<string>.Empty,
            HostLibrarySoName);
        return new SharedObjectArtifact(
            RiscVElfWriter.WriteSharedObject(program, HostLibrarySoName, initSymbol: "__host_init"),
            ExportedNames(program));
    }

    private static ImmutableArray<string> ExportedNames(RiscVProgram program)
        => program.Symbols
    ...
    private static SharedObjectArtifact BuildSharedStandardLibrary()
    {
        var program = LinkLibraryObject(
            Cnidaria.C.StandardLibrarySources.CreateFiles(),
            ImmutableArray<string>.Empty,
            StandardLibrarySoName,
            Cnidaria.C.TargetInfo.RVA32Linux);
        var image = RiscVElfWriter.WriteSharedObject(
            program,
            StandardLibrarySoName,
            initSymbol: "__libc_init");
        return new SharedObjectArtifact(image, ExportedNames(program));
    }

    private static RiscVProgram RequireProgram(Cnidaria.C.StaticLinkResult<RiscVProgram> linked, string what)
    {
        var errors = linked.Diagnostics
            .Where(static diagnostic => diagnostic.Severity == Cnidaria.C.DiagnosticSeverity.Error)
            .Select(static diagnostic => diagnostic.ToString())
            .ToArray();
        if (errors.Length != 0 || linked.Program is null)
            throw new InvalidOperationException($"Linking {what} failed: {string.Join('\n', errors)}");
        return linked.Program;
    }

    private sealed class SharedObjectArtifact
    {
        public byte[] Image { get; }
        public ImmutableArray<string> Exports { get; }

        public SharedObjectArtifact(byte[] image, ImmutableArray<string> exports)
        {
            Image = image;
            Exports = exports;
        }
...
public static class RiscVKernel
{
    ...
    public static byte[] BuildKernelImage(string startSource, string kernelSource, RiscVKernelLayout? layout = null)
    {
        ...
        var startImage = RiscVAssembler
            .Assemble(startSource, RVTarget.Rv64GVPrivileged, startSettings)
