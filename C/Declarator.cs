...
internal static class StructUnionFieldParser
{
        ...
        ImmutableArray<FieldSymbol>.Builder fields)
    {
        ...
        foreach (var rawDeclarator in SplitTopLevel(declaratorTokens, SyntaxKind.CommaToken))
        {
            ...
            {
                // An unnamed bit-field declares no member but still moves the ones that follow
                if (bitWidth.HasValue)
                    fields.Add(new FieldSymbol(string.Empty, specifiers.BaseType, containingTag, fields.Count, null, bitWidth));
                continue;
            }
    ...
    private static int FindTopLevelColon(ImmutableArray<SyntaxToken> tokens)
    {
        var parenDepth = 0;
        var bracketDepth = 0;
        var braceDepth = 0;
        ...
        for (var index = 0; index < tokens.Length; index++)
        {
            ...
            {
                case SyntaxKind.ColonToken when parenDepth == 0 && bracketDepth == 0 && braceDepth == 0:
                    return index;
