...
/// <summary>Where one member sits within its containing tag</summary>
public readonly struct FieldPlacement
{
    /// <summary>Offset of the storage the member is read and written through</summary>
    public int ByteOffset { get; }
    /// <summary>Position of the least significant bit within that storage, zero for a whole member</summary>
    public int BitOffset { get; }
    /// <summary>Declared width, or zero when the member occupies whole bytes or names nothing</summary>
    public int BitWidth { get; }
    public bool IsBitField { get; }

    private FieldPlacement(int byteOffset, int bitOffset, int bitWidth, bool isBitField)
    {
    ...
    public static FieldPlacement ForMember(int byteOffset)
        => new FieldPlacement(byteOffset, 0, 0, isBitField: false);

    public static FieldPlacement ForBitField(int byteOffset, int bitOffset, int bitWidth)
        => new FieldPlacement(byteOffset, bitOffset, bitWidth, isBitField: true);
}

/// <summary>Size, alignment and member placement of one tag, computed once for every consumer</summary>
public sealed class TagLayout
{
    public static TagLayout Empty { get; } = new TagLayout(0, 1, new Dictionary<FieldSymbol, FieldPlacement>());

    public int Size { get; }
    public int Alignment { get; }

    private readonly Dictionary<FieldSymbol, FieldPlacement> _placements;

    internal TagLayout(int size, int alignment, Dictionary<FieldSymbol, FieldPlacement> placements)
    {
        Size = size;
    ...
    public FieldPlacement PlacementOf(FieldSymbol field)
        => field is not null && _placements.TryGetValue(field, out var placement)
            ? placement
            : FieldPlacement.ForMember(0);
}
...
public sealed class TargetInfo
{
    ...
    private readonly System.Runtime.CompilerServices.ConditionalWeakTable<TagSymbol, TagLayout> _tagLayouts = new();
    ...
    private int SizeOfTag(TagSymbol symbol)
        => symbol is null || !symbol.IsComplete ? 0 : GetTagLayout(symbol).Size;

    private int AlignOfTag(TagSymbol symbol)
        => symbol is null || !symbol.IsComplete ? 1 : GetTagLayout(symbol).Alignment;

    /// <summary>Gets where every member of a tag sits, computed once and shared by every consumer</summary>
    public TagLayout GetTagLayout(TagSymbol symbol)
    {
        if (symbol is null)
            throw new ArgumentNullException(nameof(symbol));

        // Not cached: an incomplete tag gets its layout once the body closes
        if (!symbol.IsComplete)
            return TagLayout.Empty;

        return _tagLayouts.GetValue(symbol, ComputeTagLayout);
    }

    /// <summary>Gets where one member sits within its containing tag</summary>
    public FieldPlacement GetFieldPlacement(FieldSymbol field)
    {
        if (field is null)
            throw new ArgumentNullException(nameof(field));

        return GetTagLayout(field.ContainingTag).PlacementOf(field);
    }

    private TagLayout ComputeTagLayout(TagSymbol symbol)
    {
        if (!symbol.IsComplete)
            return TagLayout.Empty;

        var placements = new Dictionary<FieldSymbol, FieldPlacement>(ReferenceEqualityComparer.Instance);
        var alignment = 1;
        ...
        if (symbol.TagKind == TagKind.Union)
        {
            ...
            foreach (var field in symbol.Fields)
            {
                ...
                size = Math.Max(size, MemberStorageSize(field));
                placements[field] = field.IsBitField
                    ? FieldPlacement.ForBitField(0, 0, Math.Min(field.BitWidth!.Value, MemberStorageBits(field)))
                    : FieldPlacement.ForMember(0);
            }

            return new TagLayout(AlignTo(size, alignment), alignment, placements);
        }
        ...
        foreach (var field in symbol.Fields)
        {
            ...
            if (!field.IsBitField)
            {
                var memberAlignment = AlignOf(field.Type);
                var byteOffset = AlignTo(BytesFor(bitCursor), memberAlignment);
                placements[field] = FieldPlacement.ForMember(byteOffset);
                bitCursor = checked((byteOffset + SizeOf(field.Type)) * 8);
                continue;
            }

            var unitBits = MemberStorageBits(field);
            var width = Math.Min(field.BitWidth!.Value, unitBits);
            if (width == 0)
            {
                // A zero width names no member and closes the unit the ones before it were sharing
                bitCursor = AlignTo(bitCursor, unitBits);
                placements[field] = FieldPlacement.ForBitField(BytesFor(bitCursor), 0, 0);
                continue;
            }

            // A bit-field may not straddle a boundary of the type it was declared with
            if (checked(bitCursor % unitBits) + width > unitBits)
                bitCursor = AlignTo(bitCursor, unitBits);

            var unitOffset = checked(bitCursor / unitBits * SizeOf(field.Type));
            placements[field] = FieldPlacement.ForBitField(unitOffset, checked(bitCursor - unitOffset * 8), width);
            bitCursor = checked(bitCursor + width);
        }

        return new TagLayout(AlignTo(BytesFor(bitCursor), alignment), alignment, placements);
    }

    private int MemberStorageSize(FieldSymbol field)
        => Math.Max(1, SizeOf(field.Type));

    private int MemberStorageBits(FieldSymbol field)
        => checked(MemberStorageSize(field) * 8);
