...
{
    internal const ushort InternalCallImplFlag = 0x1000;
    public const string CProgramAssemblyName = "c";
    public const string CProgramTypeFullName = "__c.Program";
    ...
    /// <summary>Gets the ids of declared but undefined functions, which only a host handler can implement</summary>
    public IReadOnlyCollection<int> HostExternMethodIds { get; }
        ...
        IReadOnlyCollection<int>? hostExternMethodIds = null)
    {
        ...
        HostExternMethodIds = hostExternMethodIds ?? Array.Empty<int>();
        Target = target ?? TargetInfo.Default;
    }
    public RegisterBytecodeSyntheticRuntime CreateSyntheticRuntime()
        => CreateSyntheticRuntime(MethodIds, SignaturesByMethodId, EntryPc, PrintfMethodId, Target, HostExternMethodIds);
        ...
        IReadOnlyCollection<int>? hostExternMethodIds = null)
    {
        ...
        RegisterSyntheticRuntimeMethods(runtimeTypes, methodIds, signaturesByMethodId, printfMethodId, target, hostExternMethodIds);
        ...
        IReadOnlyCollection<int>? hostExternMethodIds = null)
    {
        ...
        SplitTypeFullName(CProgramTypeFullName, out var programNamespace, out var programName);
        var cProgramType = runtimeTypes.RegisterSyntheticType(CProgramAssemblyName, programNamespace, programName, RuntimeTypeKind.Class);
        foreach (var pair in signaturesByMethodId.OrderBy(static p => p.Key))
        {
            ...
            var implFlags = hostExternMethodIds is not null && hostExternMethodIds.Contains(pair.Key) ? InternalCallImplFlag : (ushort)0;
            runtimeTypes.RegisterSyntheticStaticMethod(cProgramType, name, returnType, parameters, implFlags, methodId: pair.Key);
        }
    ...
    private static void SplitTypeFullName(string full, out string ns, out string name)
    {
        ...
        ns = lastDot < 0 ? string.Empty : full.Substring(0, lastDot);
        name = lastDot < 0 ? full : full.Substring(lastDot + 1);
    }
...
public sealed class RegisterBytecodeCodeGenerator
{
    ...
    private readonly Dictionary<string, int> _hostExternMethodIdsByName = new Dictionary<string, int>(StringComparer.Ordinal);
    private readonly HashSet<int> _hostExternMethodIds = new HashSet<int>();
    private int _nextMethodId = FirstCMethodId;
    ...
    private RegisterBytecodeProgram Generate()
    {
        ...
        return new RegisterBytecodeProgram(image, entryPc, PrintfMethodId, _methodIds, _signatures, _target, _hostExternMethodIds);
    }
    ...
    /// <summary>Gives every called but undefined function an id so a host delegate can implement it</summary>
    private void IndexHostExternMethods()
    {
        for (var unitIndex = 0; unitIndex < _units.Length; unitIndex++)
        {
            foreach (var function in _units[unitIndex].Module.Functions)
                ...
                foreach (var block in function.Blocks)
                {
                    foreach (var instruction in block.Instructions)
                    {
                        if (instruction.Kind != LirInstructionKind.Call || instruction.Operands.Length == 0)
                            continue;
                        if (instruction.Operands[0].Symbol is not FunctionSymbol callee)
                            continue;
                        if (IsIntrinsicCallee(callee) || TryGetMethodId(callee, unitIndex, out _))
                            continue;

                        if (!_hostExternMethodIdsByName.TryGetValue(callee.Name, out var methodId))
                        {
                            methodId = _nextMethodId++;
                            _hostExternMethodIdsByName.Add(callee.Name, methodId);
                            _hostExternMethodIds.Add(methodId);
                            _signatures[methodId] = callee.FunctionType ?? instruction.CallSignature;
                        }

                        _methodIds.TryAdd(callee, methodId);
                    }
                }
            }
        }
    ...
    {
        _syntheticRuntime = RegisterBytecodeProgram.CreateSyntheticRuntime(_methodIds, _signatures, entryPc: 0, printfMethodId: PrintfMethodId, target: _target, hostExternMethodIds: _hostExternMethodIds);
    ...
    private void CreateMethodEntryLabels()
    {
        ...
        foreach (var methodId in _methodIds.Values.Distinct().Where(id => !_hostExternMethodIds.Contains(id)).OrderBy(static id => id))
    ...
    private sealed class FunctionEmissionContext
    {
        ...
        private void EmitCall(LirInstruction instruction)
        {
            ...
            if (TryResolveCallTarget(callee, out var methodId, out var isPrintf))
            {
                ...
                var isHostExtern = _owner._hostExternMethodIds.Contains(methodId);
                ...
                var directCallOp = CallOpForReturn(returnType, isHostExtern);
                if (isHostExtern)
                    EmitInternalCall(directCallOp, methodId, directCallFlags);
                else
        ...
        private bool TryGetCallableMethodId(FunctionSymbol function, out int methodId, out bool isPrintf)
        {
            ...
            return _owner.TryGetMethodId(function, _unitIndex, out methodId)
                   || _owner._hostExternMethodIdsByName.TryGetValue(function.Name, out methodId);
        }
        ...
        private bool TryMaterializeFunctionPointer(FunctionSymbol function, MachineRegister destination)
        {
            if (!TryGetCallableMethodId(function, out var methodId, out var isPrintf))
                return false;
            if (isPrintf || _owner._hostExternMethodIds.Contains(methodId))
                return false;
