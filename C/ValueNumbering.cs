...
public sealed class ValueNumberingOptions
{
    ...
    /// <summary>Gets whether the two operands of a commutative code may be reordered into canonical form</summary>
    internal bool CanonicalizesCommutative(GimpleTreeCode code, QualifiedType leftType, QualifiedType rightType)
    {
        if (!GimpleOperators.IsCommutative(code))
            return false;

        if (CanonicalizeCommutativeComparisons && code is GimpleTreeCode.EqExpr or GimpleTreeCode.NeExpr)
            return true;

        if (CanonicalizeCommutativeBitwiseOperators &&
            code is GimpleTreeCode.BitAndExpr or GimpleTreeCode.BitIorExpr or GimpleTreeCode.BitXorExpr)
        {
            return true;
        }

        return CanonicalizeCommutativeIntegerArithmetic &&
            code is GimpleTreeCode.PlusExpr or GimpleTreeCode.MultExpr &&
            IsIntegerLikeType(leftType) &&
            IsIntegerLikeType(rightType);
    }

    internal static bool IsIntegerLikeType(QualifiedType type)
    {
...
internal sealed class ValueNumberingBuilder
{
    ...
    private long FieldOffset(FieldSymbol field)
        => _function.Target.GetFieldPlacement(field).ByteOffset;
    ...
    private bool IsCanonicalizedCommutative(GimpleTreeCode code, QualifiedType leftType, QualifiedType rightType)
        => _options.CanonicalizesCommutative(code, leftType, rightType);

    private static bool IsIntegerLike(QualifiedType type)
        => ValueNumberingOptions.IsIntegerLikeType(type);
