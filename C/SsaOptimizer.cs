...
internal static partial class SsaOptimizer
{
    ...
    private sealed partial class Pass
    {
            ...
            ref ImmutableArray<GimpleOperandInfo> expressions)
        {
            ...
            if (NeedsCommutativeSwap(assign, values))
            {
                (values[0], values[1]) = (values[1], values[0]);
                expressions = ReplaceRhsExpressions(
                    expressions,
                    start,
                    2,
                    ImmutableArray.Create(operandArray[1], operandArray[0]));
                changed = true;
            }
        ...
        /// <summary>
        /// Reports an operand order that every backend reads worse than its mirror. A literal on the
        /// left hides the shift, the lea and the immediate encodings behind a materialized constant,
        /// because each target looks for the constant in the second operand only.
        /// </summary>
        private bool NeedsCommutativeSwap(GimpleAssignStatement assign, ImmutableArray<GimpleValue>.Builder values)
            => values.Count == 2 &&
                values[0] is GimpleConstantValue &&
                values[1] is not GimpleConstantValue &&
                _valueNumberingOptions.CanonicalizesCommutative(
                    assign.Subcode,
                    assign.Operands[0].Type,
                    assign.Operands[1].Type);
