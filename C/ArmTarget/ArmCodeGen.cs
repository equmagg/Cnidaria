...
public sealed class ArmCodeGenerator
{
    ...
    /// <summary>The AArch64 entry sequence that crt1.o provides, so a glibc image is initialised before main runs.</summary>
    private string EmitLinuxCLibraryStart(string userEntryLabel)
    {
        ...
        var startOffset = _text.ByteLength;
        _text.DefineLabel(label);
        AddExternalSymbol(LibcStartMain, ArmObjectSymbolKind.Function);

        EmitLoadImmediate(ArmRegister.X29, 0, 8);
        EmitLoadImmediate(ArmRegister.X30, 0, 8);
        Emit(ArmInstruction.Binary(ArmInstrKind.Mov, Reg(ArmRegister.X5, 8), Reg(ArmRegister.X0, 8)));
        Emit(ArmInstruction.Binary(ArmInstrKind.Ldr, Reg(ArmRegister.X1, 8), Mem(ArmRegister.Sp, 0, 8)));
        EmitAddImmediate(ArmRegister.X2, ArmRegister.Sp, 8, 8);
        EmitAddImmediate(ArmRegister.X6, ArmRegister.Sp, 0, 8);
        MaterializeSymbolAddress(userEntryLabel, ArmRegister.X0);
        EmitLoadImmediate(ArmRegister.X3, 0, 8);
        EmitLoadImmediate(ArmRegister.X4, 0, 8);
        EmitDirectCall(LibcStartMain);
        Emit(ArmInstruction.Unary(ArmInstrKind.Brk, ArmOperand.ImmediateOperand(0)));
