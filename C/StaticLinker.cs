...
public static class StaticLinker
{
        ...
        ImmutableArray<LinkerDiagnostic>.Builder diagnostics)
    {
        ...
        var userPrepared = GimplifyUnits(userSources, options, diagnostics);
        ...
        var libraryPrepared = GimplifyUnits(librarySources.ToImmutable(), options, diagnostics);
        var trimming = options.CompilationOptions.Trimming;
        if (!HasErrors(diagnostics) && trimming.Enabled && (libraryPrepared.Length != 0 || !trimming.PreserveExternallyVisibleSymbols))
        {
            TrimPrepared(userPrepared, libraryPrepared, options.EntryFunctionName, trimming);
        }

        var userUnits = CompleteUnits(userPrepared, diagnostics);
        var libraryUnits = CompleteUnits(libraryPrepared, diagnostics);
    ...
    /// <summary>
    /// Drops what the entry cannot reach before SSA. A library unit is an implementation detail of
    /// the link, so its external definitions are not roots the way a user unit's are.
    /// </summary>
    private static void TrimPrepared(
        ImmutableArray<PreparedUnit> userUnits,
        ImmutableArray<PreparedUnit> libraryUnits,
        string entry,
        TrimmingOptions trimming)
    {
        var trees = ImmutableArray.CreateBuilder<GimpleTree>();
        var unitOptions = ImmutableArray.CreateBuilder<TrimmingOptions>();
        var slots = new List<PreparedUnit>();
        var libraryTrimming = new TrimmingOptions(
            enabled: true,
            preserveExternallyVisibleSymbols: false,
            rootSymbols: trimming.RootSymbols);

        foreach (var unit in userUnits)
        {
            slots.Add(unit);
            trees.Add(unit.Gimple);
            unitOptions.Add(trimming);
        }

        var userCount = slots.Count;
        foreach (var unit in libraryUnits)
        {
            slots.Add(unit);
            trees.Add(unit.Gimple);
            unitOptions.Add(libraryTrimming);
        }

        if (trees.Count == 0)
            return;

        var trimmed = Trimmer.TrimLinked(trees.ToImmutable(), unitOptions.ToImmutable(), userCount, entry, trimming);
        for (var i = 0; i < slots.Count; i++)
            slots[i].Gimple = trimmed[i];
    }

    private static ImmutableArray<PreparedUnit> GimplifyUnits(
        ImmutableArray<SourceFile> sources,
        StaticLinkerOptions options,
        ImmutableArray<LinkerDiagnostic>.Builder diagnostics)
    {
        var units = ImmutableArray.CreateBuilder<PreparedUnit>(sources.Length);
        var compilationOptions = options.CompilationOptions;
        if (compilationOptions.Trimming.Enabled)
        {
        ...
        foreach (var source in sources)
        {
            ...
            try
            {
                units.Add(new PreparedUnit(source, semanticModel, linkage, semanticModel.GetGimpleTree()));
            }
            catch (Exception exception)
            {
                diagnostics.Add(new LinkerDiagnostic(
    ...
    private static ImmutableArray<CompiledUnit> CompleteUnits(
        ImmutableArray<PreparedUnit> units,
        ImmutableArray<LinkerDiagnostic>.Builder diagnostics)
    {
        var completed = ImmutableArray.CreateBuilder<CompiledUnit>(units.Length);
        foreach (var unit in units)
        {
            try
            {
                completed.Add(new CompiledUnit(
                    unit.Source,
                    unit.SemanticModel,
                    unit.Linkage,
                    GimplePipeline.Run(unit.Gimple)));
            }
            catch (Exception exception)
            {
                diagnostics.Add(new LinkerDiagnostic(
    ...
    private sealed class PreparedUnit
    {
        public SourceFile Source { get; }
        public SemanticModel SemanticModel { get; }
        public FileScopeLinkageMap Linkage { get; }
        public GimpleTree Gimple { get; set; }
