...
public sealed class X86CodeGenerator
{
    ...
    private const string LibcStartMain = "__libc_start_main";

    private string EmitLinuxRuntime(string userEntryLabel)
    {
        if (_options.CLibraryStartup && _machineTarget.Is64Bit)
            return EmitLinuxCLibraryStart(userEntryLabel);
    ...
    /// <summary>The System V entry sequence that crt1.o provides, so a glibc image is initialised before main runs.</summary>
    private string EmitLinuxCLibraryStart(string userEntryLabel)
    {
        if (string.IsNullOrEmpty(userEntryLabel))
            throw new InvalidOperationException("A C library entry sequence needs an entry function.");
        ...
        Emit(X86Instruction.Binary(X86InstrKind.Mov, Reg(X86Register.R9, 8), Reg(X86Register.Rdx, 8)));
        Emit(X86Instruction.Unary(X86InstrKind.Pop, Reg(X86Register.Rsi, 8)));
        Emit(X86Instruction.Binary(X86InstrKind.Mov, Reg(X86Register.Rdx, 8), Reg(X86Register.Rsp, 8)));
        Emit(X86Instruction.Binary(X86InstrKind.And, Reg(X86Register.Rsp, 8), Imm(-16)));
        Emit(X86Instruction.Unary(X86InstrKind.Push, Reg(X86Register.Rax, 8)));
        Emit(X86Instruction.Unary(X86InstrKind.Push, Reg(X86Register.Rsp, 8)));
        Emit(X86Instruction.Binary(X86InstrKind.Xor, Reg(X86Register.R8, 4), Reg(X86Register.R8, 4)));
        Emit(X86Instruction.Binary(X86InstrKind.Xor, Reg(X86Register.Rcx, 4), Reg(X86Register.Rcx, 4)));
        Emit(X86Instruction.Binary(X86InstrKind.Lea, Reg(X86Register.Rdi, 8), X86Operand.RipRelative(userEntryLabel, 0, 8)));
        Emit(X86Instruction.Branch(X86InstrKind.Call, X86Operand.SymbolOperand(LibcStartMain, 4, X86ObjectRelocationKind.Relative32)));
        Emit(new X86Instruction(X86InstrKind.Ud2));

        _symbols.Add(new X86ObjectSymbol(
            label, TextSectionName, startOffset, _text.ByteLength - startOffset, X86ObjectSymbolBinding.Global, X86ObjectSymbolKind.Function));
        return label;
    }
    ...
    {
        if (!_machineTarget.Is64Bit ||
            instruction.Opcode != X86InstrKind.Mov ||
            instruction.Operand0.Kind != X86OperandKind.Register ||
            instruction.Operand1.Kind != X86OperandKind.Symbol ||
            instruction.Operand1.RelocationKind != X86ObjectRelocationKind.Absolute64)
        {
            return instruction;
        }
        ...
        return X86Instruction.Binary(
            ...
            X86Operand.RipRelative(instruction.Operand1.Symbol!, instruction.Operand1.Addend, 8));
    }
            ...
            out X86Operand operand)
        {
            ...
            switch (root.Kind)
            {
                ...
                case LirAddressKind.Symbol:
                    ...
                    if (_owner._options.IndirectExternalData && _owner._externalLabels.Contains(ripSymbol))
                        return false;
